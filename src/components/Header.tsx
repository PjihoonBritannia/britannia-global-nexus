...
const Header = () => {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-300 ${
      isScrolled || (isMenuOpen && !isScrolled) ? 'bg-white/90 backdrop-blur-md shadow-md py-4' : 'bg-transparent py-6'
    }`}>
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        >
          <img 
            src={isScrolled || (isMenuOpen && !isScrolled) ? "/logo_black.svg" : "/logo_white.svg"} 
            alt="Britannia Inc." 
          ...
          {isScrolled && (
            <Link 
              ...
              className={`font-light link-underline transition-colors duration-200 ${
                location.pathname === '/' ? 'text-point' : 'text-base-dark'
              }`}
            >
              Home
            </Link>
          ...
          <nav className="container mx-auto px-6 py-5 flex flex-col space-y-6">
            <Link 
              ...
              className={`font-light hover:text-point p-2 transition-all duration-200 ${
                location.pathname === '/' ? 'text-point translate-x-2' : 'text-base-dark'
              }`}
